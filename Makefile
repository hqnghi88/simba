...
up: setup-network
	...
	@if [ "${DEVICE}" = "cuda" ]; then \
		...
		DEVICE=cuda RUNTIME=nvidia RUN_OLLAMA=ollama COMPOSE_PROFILES=ollama docker compose -f docker/docker-compose.yml up -d; \
	elif [ "${ENABLE_OLLAMA}" != "false" ]; then \
		echo "Enabling Ollama without GPU"; \
		DEVICE=${DEVICE} RUNTIME="" RUN_OLLAMA=ollama COMPOSE_PROFILES=ollama docker compose -f docker/docker-compose.yml up -d; \
	else \
...
.PHONY: setup-network setup-builder build up down clean logs help docker-prune push-docker migrate run-local
