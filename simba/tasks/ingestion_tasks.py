...
def ingest_document_task(self, file_path: str, file_name: str, file_size: int, folder_path: str = "/"):
    ...
    try:
        ...
        # Trigger summary generation immediately for RAG
        try:
            from simba.tasks.generate_summary import generate_summary_task
            generate_summary_task.delay(simba_doc.model_dump())
            logger.info(f"Triggered summary generation for {simba_doc.id}")
