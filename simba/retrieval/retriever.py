...
class Retriever:
    ...
    def retrieve(
        ...
        """
        # Try to get user_id from supabase, handle failure/mock gracefully
        user_id = "mock-user-id" # Default for local/mock
        try:
            supabase = get_supabase_client()
            # Check if it's a real client and has auth
            if hasattr(supabase, "auth") and not supabase.__class__.__name__ == "MockSupabaseClient":
                 user = supabase.auth.get_user()
                 if user and user.user:
                     user_id = user.user.id
        except Exception as e:
            logger.warning(f"Failed to get user_id from supabase (using mock): {e}")
